...
import logging, telegram, datetime, sqlite3, urllib.request, json
from data import *
...
def hourly_log(bot, job): 
    ...
    for location in places.keys():
        ...
        cursor.execute('INSERT INTO {} VALUES("{}",{},{},{});'.format(
            places[location]["today_db"], datetime.datetime.now().strftime('today %H:%M'), 
            "{0:.1f}".format(owm), 
            "{0:.1f}".format(darksky), 
            "{0:.1f}".format(apixu)))
        connection.commit()
...
def main():
    ...
    dp.add_handler(CommandHandler("история", history_and_today, pass_args=True))
